...
net = Net()
net.load_state_dict(torch.load('emnist_model.pt'))
net.eval()
...
for img in os.listdir('../images'):
    ...
    crops = np.zeros((len(bboxes),28,28))
    crop_count = 0
    for line in lines:
        for index in line:
            ...
            # resize image
            rect = skimage.transform.resize(rect,(28,28))
            rect = rect.T
            # plt.imshow(rect,cmap=plt.cm.gray);plt.show()
            # crop = (rect.T).flatten()
            crops[crop_count,:,:] = rect
            crop_count += 1
    ...
    text = ''
    for crop in crops:
        # plt.imshow(crop,cmap=plt.cm.gray);plt.show()
        inputs = torch.from_numpy(crop).float()
